...
这些 `mal::remote::` 函数在交互库内部实现，选手只复制声明，不复制实现。整数
侧（`+ - * / %`、`nroot`）直接跑在以 `10^5` 为一块的十进制核心上，读写不做
进制换算；小数侧与二进制接口（位运算等）仍由 `mal::BigFloat` / `mal::BigInt`
承担。
...
| 窄商除法 | 截断除数 + 精度倍增倒数 |
| 一般除法 | Knuth D |
| `nroot` | 精度倍增初值 + 单步 Newton 细化 + 相邻幂校正 |

**交互库有两种。** `bundles/interactive_lib.cpp` 是含全部模块的完整版
（`hp` / `dgf` / `poly` / `number_theory` / `common` / `remote` 都在里面）；
`g++ -std=c++14 -O2 scripts/build.cpp -o /tmp/mal_build && /tmp/mal_build --all include bundles`
还会顺带生成题目专用版 `examples/T793310/interactive_lib.cpp`：它只打包远程
接口和它依赖的模块，并把库内部标识符缩成短名（文档里承诺的公开名字一律保留），
体积约为完整版的三分之二，链接行为与完整版一致。
...
NTT 用双模数 + CRT 还原，结果精确无误差。切换规模依机器而定，代码里是常量。
...
在示例题 T793310 当时那批评测数据上实测（洛谷评测机，C++14 / GCC 9 / `-O2`；
10 组数据，这批最大一组是 200 万位除以 198 万位）：

| 提交 | 用时 | 内存 |
|---|---:|---:|
| 使用 MAL（本库，开启 AVX2） | `1.79 s` | `69.29 MB` |
| PyPy 3 参考实现 | `5.57 s` | `166.38 MB` |

逐测试点：

| 测试点 | 内容 | MAL | PyPy 3 |
|---:|---|---:|---:|
| 1 | 7 种运算，30 位 | `4 ms / 788 KB` | `162 ms / 49.46 MB` |
| 2 | 10 次 1000 位乘法 | `5 ms / 788 KB` | `156 ms / 48.91 MB` |
| 3 | 5 次 4 万位乘法 | `118 ms / 5.32 MB` | `258 ms / 52.93 MB` |
| 4 | 4 万位开 `k` 次根 | `101 ms / 2.70 MB` | `249 ms / 52.33 MB` |
| 5 | `exp` / `log`（含 100 万位 `log`） | `7 ms / 2.46 MB` | `170 ms / 54.08 MB` |
| 6 | 5 万位加减 | `90 ms / 3.12 MB` | `217 ms / 52.64 MB` |
| 7 | 3 次 10 万位乘法 | `196 ms / 10.21 MB` | `396 ms / 61.42 MB` |
| 8 | 30 万位开三次根 | `207 ms / 13.91 MB` | `939 ms / 103.44 MB` |
| 9 | 160 万 ÷ 158 万位 | `487 ms / 65.39 MB` | `1.31 s / 135.37 MB` |
| 10 | 200 万 ÷ 198 万位 | `572 ms / 69.29 MB` | `1.71 s / 166.38 MB` |

![洛谷提交对比](assets/perf-luogu-overview.png)

![MAL 各测试点](assets/perf-luogu-mal-cases.png)

![PyPy 3 各测试点](assets/perf-luogu-pypy-cases.png)

不支持 AVX2 的机器会自动回退到标量实现，结果完全一致，只是慢一些。

上表是当时那批数据的评测记录；示例题的数据后来加大过两次，目前最大一组是
800 万 ÷ 792 万位，规模见 `examples/T793310/README.md`。
