...
数据刻意开到较大规模（最大一组是 800 万位除以 792 万位），用来考察 MAL 在
超大位数下是否仍然可用。
...
最大几组的位数：开三次根到 120 万位，两次除法到 640 万÷632 万位与 800 万÷792 万位。
...
| 2 | 10 次 2000 位整数乘法 |
| 3 | 5 次 20 万位整数乘法 |
| 4 | 20 万位整数开 `k` 次根，`k = 2..5` |
| 5 | `exp` / `log`，含一条 `2×10^6` 位整数的 `log` |
| 6 | 20 万位整数加减 |
| 7 | 3 次 40 万位整数乘法 |
| 8 | 120 万位整数开三次根 |
| 9 | 640 万 ÷ 632 万位 |
...
`data.zip` 的根目录只包含 `interactive_lib.cpp` 和 10 组 `.in` / `.ans`。其中的
交互库是本题专用版：只打包接口用得到的模块，库内部标识符缩成短名，公开名字
（选手会复制的那些声明）不变，源码体积约 93 KB，完整库见
...
/tmp/mal_build --all include bundles      # 同时生成本题的 interactive_lib.cpp
python3 examples/T793310/generate_interface.py
CXX=g++ python3 examples/T793310/generate_data.py
```

第一步会把 `include/` 下的头文件合并成两份交互库：`bundles/interactive_lib.cpp`
是含全部模块的完整版；`examples/T793310/interactive_lib.cpp` 是本题专用的精简版，
只打包远程接口和它用到的模块，并把库内部标识符缩成短名（公开名字不变），
体积约为完整版的七成。`generate_interface.py` 只负责重新生成 `main.cpp`。
