...
本版把最大几组的位数调大了一次（开三次根到 30 万位、两次除法到 160 万÷158 万
位与 200 万÷198 万位），好让优化在同一份数据上看得清楚。

洛谷实测（**上一版较小数据**上开启 AVX2 的版本；时间为该组评测时间，内存为峰值，
用来对照 AVX2 的收益；本次加大后的数据待重测）：

| 编号 | 内容 | 时间 | 峰值内存 |
|---:|---|---:|---:|
| 1 | 7 种运算各一条，30 位 | `5 ms` | `812 KB` |
| 2 | 10 次 1000 位乘法 | `50 ms` | `2.43 MB` |
| 3 | 5 次 4 万位乘法 | `44 ms` | `1.55 MB` |
| 4 | 4 万位开 `k` 次根，`k = 2..5` | `7 ms` | `2.65 MB` |
| 5 | `exp` / `log`（含 100 万位整数的 `log`） | `37 ms` | `1.64 MB` |
| 6 | 5 万位整数加减 | `82 ms` | `4.43 MB` |
| 7 | 3 次 10 万位乘法 | `110 ms` | `7.06 MB` |
| 8 | 30 万位整数开三次根 | `221 ms` | `29.17 MB` |
| 9 | 160 万 ÷ 158 万位 | `247 ms` | `30.18 MB` |
| 10 | 200 万 ÷ 198 万位 | 待补 | 待补 |

同一份数据在**未开启 AVX2** 时（第 2/3/5/6/7/8/9 组）分别是
`81 / 65 / 54 / 141 / 217 / 368 / 436 ms`，即 AVX2 版本快 `1.46 ~ 1.97` 倍；
不支持 AVX2 的机器会自动回退到这套标量实现，结果完全一致。

下面这张表是最初那版较小数据的对比，保留作历史参考：
