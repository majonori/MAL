...
数据刻意开到让最快的实现也要跑几百毫秒，方便直观感受 MAL 的速度。
...
| `*` | 高精度整数乘法 | 两个整数 `a b` | `a * b` |
| `/` | 高精度整数除法 | 两个整数 `a b`（`b != 0`） | `a / b`，向零取整 |
| `exp` | 自然指数 | 一个实数 `x` | `e^x` |
...
|---|---|
| `T` | `1 <= T <= 10` |
| `+`、`-` | 操作数不超过 `2.5 * 10^4` 位十进制 |
| `*` | 每个操作数不超过 `5 * 10^4` 位十进制 |
| `/` | 被除数不超过 `10^6` 位十进制，除数与之同量级 |
| `exp`、`log` | 参数不超过 30 位有效数字；另有一组 `log` 的参数是 `10^6` 位整数 |
| `nroot` | `a` 不超过 `1.5 * 10^5` 位十进制，`2 <= k <= 5` |

提示：

- `mal::BigInt` 支持 `+ - * / %`、比较、`pow`、`sqrt`、`nroot`、`<< >>` 等；
  `mal::BigFloat` 支持 `+ - * /`、比较、`exp`、`log`、`sqrt`、`pow`、`pi`、`ln2` 等，
  两种类型都能直接 `std::cin >>` / `std::cout <<`；
- `mal::BigFloat` 的第二个参数是二进制有效位数，默认 256；输出 30 位有效数字用
  `to_string(30)`；
- 整数除法向零取整，例如 `-7 / 2 = -3`；
- 最慢的一组数据（两个 `2 * 10^5` 位整数相乘）用 MAL 跑约 `0.5` 秒，
  建议时间限制 2 秒、空间限制 512 MB；
- 复制粘贴的完整教程见仓库 `TUTORIAL.md`。
