...
| 运算 | 范围 |
|---|---|
| `+`、`-` | 操作数不超过 $2\times10^5$ 位十进制 |
| `*` | 每个操作数不超过 $4\times10^5$ 位十进制 |
| `/` | 每个操作数不超过 $8\times10^6$ 位 |
| `exp`、`log` | 参数不超过 30 位有效数字；另有一组 `log` 的参数是 $2\times10^6$ 位整数 |
| `nroot` | $a$ 不超过 $1.2\times10^6$ 位十进制，$2\le k\le5$ |
